...
//Рисуем квадрат
function square() {
	ctx.fillRect(RECT_SIZE/2, RECT_SIZE/2, RADIUS_LENGTH * 2, RADIUS_LENGTH * 2)
}
...
function triangle(){
	...
	ctx.lineTo(RECT_SIZE/2 - RADIUS_LENGTH, RECT_SIZE/2)
...
canvas.addEventListener("mousedown", e => {
	...
	draw()
	$("#x_input").val(arrayOfCoords[0].x.toFixed(4))
	document.querySelectorAll("input[type='checkbox']").forEach(elem =>{
		elem.checked = false;
		if (elem.value == arrayOfCoords[0].y){
			elem.checked = true;
		}
	})
	//arrayOfCoords.push(new Coordinate(xFromCanvasToNoraml(xCor), yFromCanvasToNoraml(yCor), rCor, true)) //toDo
	r_error.innerHTML = ""
	x_error.innerHTML = ""
	y_error.innerHTML = ""
})
...
document.querySelectorAll("input[type='checkbox']").forEach((elem)=>{
	elem.addEventListener("change", e =>{
		validateY()
		arrayOfCoords = []
		document.querySelectorAll("input[type='checkbox']:checked").forEach((elems)=>{
...
/*
function isThereNewCoordFormGraph(){
	let arrayOfNewCoords = [];
	arrayOfCoords.forEach(elem => {
		if (elem.isNew = true) {
			arrayOfNewCoords.push(elem)
		}
	})
	if (arrayOfNewCoords.length != 0){
...
function validation() {
	if (validateX() && validateR() && validateY()){
		...
		/*document.querySelectorAll("input[type='checkbox']:checked").forEach(elem => {
			x_val = $("input[type='text']").val()
			y_val = elem.value
			r_val = radius
			//arrayOfCoords.push(new Coordinate(Number.parseInt(x_val).toFixed(3), y_val, r_val, false))
			//drawAllCoords()
			//sendRequest(x_val, y_val, r_val)
		})*/
	}
